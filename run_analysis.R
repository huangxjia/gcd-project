...
##[grep("mean", colnames(totalMerge5))]
s2 <- gsub("-mean()-", "Mean", s1, fixed = TRUE)
s3 <- gsub("-mean()", "Mean", s2, fixed = TRUE)

s6 <- gsub("tBody", "TimeOFBody", s3, fixed = TRUE)
s7 <- gsub("tGravity", "TimeOFGravity", s6, fixed = TRUE)
s8 <- gsub("fBodyBody", "FrequencyOFBody", s7, fixed = TRUE)
s9 <- gsub("fBody", "FrequencyOFBody", s8, fixed = TRUE)
s10 <- gsub("Acc", "Acceleration", s9, fixed = TRUE)
s11 <- gsub("Gyro", "Gyroscope", s10, fixed = TRUE)
s12 <- gsub("Mag", "Magnitude", s11, fixed = TRUE)
